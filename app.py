...
def render_landing_page():
    """Renders the educational landing page."""
...
def main():
    if "vm_last_result" not in st.session_state: st.session_state["vm_last_result"] = None
    if "seg_last_result" not in st.session_state: st.session_state["seg_last_result"] = None
    if "vm_show_compare" not in st.session_state: st.session_state["vm_show_compare"] = False
    if "anim_trigger" not in st.session_state: st.session_state["anim_trigger"] = False

    # --- MODE SELECTION ---
    with st.sidebar:
        st.markdown("## 🖥️ Mode Selection")
        app_mode = st.radio("Choose Simulator:", ["Paging", "Segmentation"], horizontal=True)
        st.divider()
    ...
    # ==========================================
        ...
        st.title("🧱 Segmentation Simulator")
        
        with st.sidebar:
            st.markdown("## ⚙️ Seg. Config")
            total_ram = st.number_input("Total RAM Size (KB)", 100, 10000, 1000, step=100)
            
            algo_choice = st.selectbox("Allocation Strategy", ["First Fit", "Best Fit", "Worst Fit"])
            
            st.info("Input Format:\n- Alloc: `ID:Size` (e.g., `1:100`)\n- Dealloc: `-ID` (e.g., `-1`)")
            seg_input = st.text_area("Operations Sequence", 
                                     value="1:200 2:300 -1 3:400 4:100 -2 5:250", 
                                     height=150,
                                     help="Space separated. Example: 1:200 allocates 200KB for PID 1. -1 removes PID 1.")
            
            run_seg = st.button("🚀 Run Segmentation", use_container_width=True)

        if run_seg:
            reqs = seg_input.split()
            res = segmentation_allocator(reqs, total_ram, strategy=algo_choice)
            st.session_state["seg_last_result"] = res
            st.session_state["seg_renderer_cur"] = 0 
            
        if st.session_state["seg_last_result"]:
            res = st.session_state["seg_last_result"]
            renderer = SegmentRenderer(key="seg_renderer")
            renderer.render(res, total_ram)
        else:
            st.markdown("""
            ### How Segmentation Works
            1. **Variable Sizing:** Unlike Paging, segments can be any size (e.g., Code, Stack).
            2. **Strategies:**
                * **First Fit:** Uses the first available hole. Fast.
                * **Best Fit:** Uses the smallest hole that fits. Saves large holes.
                * **Worst Fit:** Uses the largest hole. Reduces tiny useless gaps.
            3. **External Fragmentation:** Notice the gaps (checkerboard pattern) that form between processes.
            
            👈 **Enter a sequence in the sidebar to start!**
            """)
